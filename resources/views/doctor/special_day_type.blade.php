...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>Doctor Special Day Type</span>
            </li>
        </ul>
    </div>
    <!-- END PAGE BAR -->
	      			...
	      			<div class="portlet light bordered">
						<div class="portlet-title">
				            <div class="caption">
				                ...
				                <span class="caption-subject font-dark bold uppercase"><i class="fa fa-table"></i> Doctor Special Day Type</span>
				            </div>

				            <div class="actions">
								@if(\App\Utility::userRolePermission(Session::get('role_id'),64))
				        ...
				        <div class="portlet-body Details">
							...
							@if(\App\Utility::userRolePermission(Session::get('role_id'),63))
				        	<div class="portlet light bordered">
...
function delData(a,b){
	...
	if(b==1){$("#confirm").html('Are you sure you want to active this doctor special day type?');}
	if(b==2){$("#confirm").html('Are you sure you want to inactive this doctor special day type?');}
...
$("#delData").click(function(){
	...
	var url = '{{ url('/deldoctorSpecialDayType') }}';
	var dataString = {
	  _token: "{{ csrf_token() }}",
	  id:a,status:b
	};
...
$("#edit_data").click(function(){
	var url = '{{ url('/doctorSpecialDayTypeUpdate') }}';
...
//Edit val Ajax
function editData(a){
    var url = '{{ url('/getdoctorSpDayEditVal') }}';
	var dataString = {
	  _token: "{{ csrf_token() }}",
	  id:a
	};
...
$("#save_date_type").click(function(){
	var url = '{{ url('/doctorSpecialDayStore') }}';
	var name = $('#name').val();
...
function getData(){
	var url = "{{ url('/getdoctorSpecialDayType') }}";
		var dataString = {
            _token: "{{ csrf_token() }}",
        };
            ...
            success: function(data){
                ...
                $('#data_list').DataTable().destroy();
                	...
                	if(value.name !== null){ var name = value.name}else{ var name = ''; }
						...
						@if(\App\Utility::userRolePermission(Session::get('role_id'),65))
				        html += '<a title="Edit" href="javascript:;" class="btn btn-sm btn-info" onclick="editData('+value.doctor_special_day_type_id+')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>';
				        @endif

						@if(\App\Utility::userRolePermission(Session::get('role_id'),66))
						if(value.status == 'active'){
						html += '<a title=" Inactive" href="#" class="btn btn-sm btn-danger" onclick="delData('+value.doctor_special_day_type_id+','+2+')"><i class="fa fa-ban" aria-hidden="true"></i></a>';
						}else{
						html += '<a title="Active" href="#" class="btn btn-sm btn-success" onclick="delData('+value.doctor_special_day_type_id+','+1+')"><i class="fa fa-flash" aria-hidden="true"></i></a>';
						}
						@endif
