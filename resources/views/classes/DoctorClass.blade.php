...
@section('content')
	      			...
	      			<div class="portlet light bordered">
						<div class="portlet-title">
				            <div class="caption">
				                ...
				                <span class="caption-subject font-dark bold uppercase"><i class="fa fa-table"></i> Create Doctor Class</span>
				            </div>

				            <div class="actions">
								@if(\App\Utility::userRolePermission(Session::get('role_id'),52))
                                <div class="btn-group btn-group-devided">
                                    <button type="button" class="btn btn-primary" id="add_new_class" data-toggle="modal" data-target="#addDoctorClass"><i class="icon-plus icons"></i>Add New</button>
                                </div>
								@endif
                            </div>
				        </div>
				        ...
				        <div class="portlet-body Details">
							...
							@if(\App\Utility::userRolePermission(Session::get('role_id'),51))
				        	<div class="portlet light bordered">
				                        ...
				                        <table class="table table-bordered datatable" id="class_list" width="100%" cellspacing="0">
							              	<thead>
								                <tr>
								                  ...
								                  <th>Doctor Class Name</th>
								                  <th class="text-center w250">Action</th>
								                </tr>
							              	</thead>
							              	<tbody id="set_Doctor_class">
	...
	<!-- Modal -->
	<div id="addDoctorClass" class="modal fade" role="dialog">
		    ...
		    <div class="modal-content">
				...
				<!-- Alert Massage Section -->
		        	...
		        	<div class="row">
		        		<div class="col-sm-12">
		        			<div class="form-group form-md-line-input form-md-floating-label">
		                        ...
		                        <label for=""> Doctor Class Name</label>
		                        <span class="help-block">Enter Doctor Class Name...</span>
		                    </div>
		        		</div>
		        	</div>
		      	</div>
		      	<div class="modal-footer">
		        	...
		        	<button type="button" class="btn btn-success" id="saveDoctorClass">Save</button>
		      	</div>
		    </div>
	...
	<!-- Product Edit Modal -->
	<div class="modal fade" id="DoctorClassEditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  	...
	  	<div class="modal-dialog">
		    ...
		    <div class="modal-content">
		      	...
		      	<div class="modal-footer">
		        	...
		        	<a href="#" data-id="" data-status="" class="btn btn-success" id="delDocSpeciality">Yes, Continue</a>
		      	</div>
		    </div>
...
//Del product Ajax
function delDoctorClass(a,b){
	$('#activeInactive').modal('show');
	if(b==1){$("#confirm").html('Are you sure you want to active this doctor class?');}
	if(b==2){$("#confirm").html('Are you sure you want to inactive this doctor class?');}
    $("#delDocSpeciality").attr('data-id',a);
    $("#delDocSpeciality").attr('data-status',b);
}

$("#delDocSpeciality").click(function(){
	var a = $("#delDocSpeciality").attr('data-id');
	var b = $("#delDocSpeciality").attr('data-status');
	var url = '{{ url('/Doctor/delDoctorClass') }}';
	var dataString = {
	  _token: "{{ csrf_token() }}",
	  id:a,status:b
	};
...
$("#editClass").click(function(){
	var url = '{{ url('/Doctor/DoctorClassUpdate') }}';
        ...
        $.ajax({
            ...
            success: function(data){ console.log(data);
                if(data.status == 200){
					...
					setTimeout(function() {$('#DoctorClassEditModal').modal('hide');},2000);
					getData();
                }
...
//Save product Ajax
function editDoctorClass(a){
    var url = '{{ url('/DoctorClass/getEditVal') }}';
	var dataString = {
	  _token: "{{ csrf_token() }}",
	  id:a
	};
    $.ajax({
        ...
        success: function(data){ console.log(data);
              if(data.status == 200){
              	...
              	$('#DoctorClassEditModal').modal('show');
...
//Save product Ajax
$("#saveDoctorClass").click(function(){
	var url = '{{ url('/Doctor/DoctorClassStore') }}';
	var name = $('#name').val();
        ...
        $.ajax({
            ...
            success: function(data){ console.log(data);
                if(data.status == 200){
					...
					setTimeout(function() {$('#addDoctorClass').modal('hide');},2000);
					getData();
                }
                if(data.status == 404){
					$("#error").css('display','block');
...
function getData(){
	var url = "{{ url('/Doctor/getDoctorClass') }}";
		var dataString = {
            _token: "{{ csrf_token() }}",
        };
            ...
            success: function(data){
                ...
                $('#class_list').DataTable().destroy();
                	...
                	if(value.class_name !== null){ var name = value.class_name}else{ var name = ''; }
						...
						@if(\App\Utility::userRolePermission(Session::get('role_id'),53))
				        html += '<a title="Edit" href="javascript:;" class="btn btn-sm btn-info" onclick="editDoctorClass('+value.class_id+')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>';
				        @endif

						@if(\App\Utility::userRolePermission(Session::get('role_id'),54))
						if(value.status == 'active'){
						html += '<a title=" Inactive" href="#" class="btn btn-sm btn-danger" onclick="delDoctorClass('+value.class_id+','+2+')"><i class="fa fa-ban" aria-hidden="true"></i></a>';
						}else{
						html += '<a title="Active" href="#" class="btn btn-sm btn-success" onclick="delDoctorClass('+value.class_id+','+1+')"><i class="fa fa-flash" aria-hidden="true"></i></a>';
						}
						@endif
